...
var find ="select name,text from task where name = '" + req.body.name + "' and text = '" + req.body.text + "'";
  var insertSql ="insert into task(name,text,downtime,money,photo,tel,address) values(?,?,?,?,?,?,?)";
let a =[req.body.name,req.body.text,req.body.downtime,req.body.money,req.body.photo,req.body.tel,req.body.address];
...
router.post('/', function (req, res, next) {
  ...
  var insql = 'select * from task where name=? or text=?  or downtime=? or money=? or photo=? or tel=? or address=?';
   
  connection.query(insql, [req.body.sea,req.body.sea,req.body.sea,req.body.sea,req.body.sea,req.body.sea,req.body.sea], function (err, result, fields) {
    if (err) {
      console.log('err', err);
      return;
    } else { 
